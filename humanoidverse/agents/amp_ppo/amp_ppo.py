...
from humanoidverse.utils.normalize import AMPNormalizer
...
class AMPPPO(BaseAlgo):
    ...
    def setup(self):
        ...
        logger.info("Setting up AMP-PPO")
        # 设置actor和critic网络和优化器
        self._setup_models_and_optimizer()
        logger.info(f"Setting up Storage")
        self._setup_storage()
        # 设置AMP的模型和优化器
        self._setup_amp_models_and_optimizer()
    ...
    def _init_loss_dict_at_training_step(self):
        ...
        loss_dict['AMP'] = 0
        loss_dict['GradPen'] = 0
        loss_dict['PolicyPred'] = 0
        loss_dict['ExpertPred'] = 0
        return loss_dict
    
    def _update_algo_step(self, policy_state_dict, loss_dict, sample_amp_policy=None, sample_amp_expert=None):
        # 先更新actor、critic网络
        ppo_loss_dict = self._update_ppo(policy_state_dict, loss_dict)
        # 再更新discriminator网络
        amp_loss_dict = self._update_amp_ppo(loss_dict, sample_amp_policy, sample_amp_expert)
