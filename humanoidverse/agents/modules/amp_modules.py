    ...
    def _process_module_config(self, obs_dim_dict, module_config_dict):
        ...
        for idx, output_dim_placeholder in enumerate(module_config_dict['output_dim']):
            if output_dim_placeholder == 'discriminator_output_dim':
                module_config_dict['output_dim'][idx] = 1
                
        return module_config_dict

    @property
    def discriminator(self):
        return self.discriminator_module

    def forward(self, x):
        return self.discriminator_module(x)
    
    def compute_grad_pen(self, expert_state, expert_next_state, lambda_=10):
        # Compute gradient penalty for WGAN-GP
        alpha = torch.rand(expert_state.size(0), 1, device=expert_state.device)
        alpha = alpha.expand_as(expert_state)
        
        interpolated_state = alpha * expert_state + (1 - alpha) * expert_next_state
        interpolated_next_state = alpha * expert_next_state + (1 - alpha) * expert_state
        
        interpolated = torch.cat([interpolated_state, interpolated_next_state], dim=-1)
